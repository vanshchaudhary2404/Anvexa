...
const Checkout = () => {
  ...
  //IMportant: Razorpay integration requires backend support. Ensure your backend is configured with Razorpay keys and endpoints for order creation and payment verification. 
  const handlePayment = async () => {
    try {
      ...
      if (!orderRes.ok) {
        ...
        if (fallback) {
          toast.warning("Razorpay is not configured. Using Student Bypass Mode.");
  ...
  const bypassPayment = async () => {
    ...
    if (saveOrderRes.ok) {
    dispatch(clearCart());
    toast.success('🎉 Your order has been placed successfully! in bypass mode');
    navigate('/ordersuccess');
    } else {
  ...
  return (
    ...
    <h2 style={{ textAlign: 'center', width: '100%' }}>Checkout</h2>
